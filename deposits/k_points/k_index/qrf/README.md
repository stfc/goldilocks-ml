...
`q3bye-wep37`, which is a spacing in Å⁻¹ that a consumer converts into a mesh
through the reciprocal lattice. A k-index needs no conversion: the answer *is*
the position in a table the consumer already has.
...
| --- | --- |
| A rung too low | The mesh is too coarse. The calculation is under-converged, and the answer is wrong without looking wrong. |
...
the cheapest rule keeping under-prediction at or below **6%**, and are recorded
in `model.json` under `decision`. A consumer applies nothing further: the number
in the prediction is the rung to use.

## Training data

PSDI record [`d5ds2-64f16`](https://data-collections.psdi.ac.uk/records/d5ds2-64f16),
CC BY 4.0. The record holds 18220 MC3D structures with Quantum ESPRESSO k-mesh
convergence studies; **17757 of them converged and carry a label**, and only
those were used. The other 463 have a structure and no answer.

Labels run from rung 0 to rung 41 and are heavily skewed: half the dataset sits
at rung 3 or below, 0.6% above rung 20, and five rungs below 41 have no example
at all.

The split is 70/10/10/10 train/validation/calibration/test, seed 42, **grouped
by reduced composition** — 15712 groups over 17757 structures, so two
polymorphs of one composition cannot land on opposite sides of it. No group
spans more than one split.
...
answers silently, so verify the names in `model.json` rather than assuming them.

Matminer cannot compute a packing fraction for an element with no tabulated
atomic radius. **45 of the 17757 training structures contain He, Ne, Kr or Xe**
and their whole 7-column structure block was written as zeros. Any consumer
must apply the same fallback, deterministically, or those crystals will be
described differently at inference than they were at training.
...
| | Too coarse | Mean excess rungs | Exact rung | MAE | R² |
| --- | ---: | ---: | ---: | ---: | ---: |
...
**Read the first two columns.** The floor was set at 6% on validation, and the
test split, which chose nothing, comes in at 4.4%.

**MAE and R² are not measuring accuracy here.** They price a deliberate bias as
if it were error. The correlation with the truth is 0.828 for the published
value against 0.858 for the median: the model's ability to rank structures by
the mesh they need is intact, and what changed is where the number sits
relative to the truth. Of the published mean squared error of 14.54, some 5.88
is the deliberate lift — a *perfect* rule sitting exactly 2.42 rungs above every
true rung would itself score R² 0.612 against a target variance of 15.17.
...
**The top of the ladder is not reliable.** Banded on the *true* rung rather
than the published one, structures that genuinely need rung 11 or above are
...
surface and none of the 174 columns knows whether the crystal is a metal. This
...
`model.json` also pins the same digest, and `goldilocks-ml`'s loader refuses to
...
The estimator returns 11 quantile levels in the order recorded in
`model.json` under `levels`. The published rung comes from the `decision` block;
the 5th and 95th are the interval.

## The interval is a diagnostic, not the answer

`model.json` records a 90% interval and a split-conformal calibration whose
correction came out at exactly **0.0** — the raw interval already covered 94.8%
...
Test coverage is 0.953 overall but **0.805** for structures whose true rung is
11 or above, at a mean width of 10.5 rungs. Do not read the interval as a
uniform guarantee; the published rung, not the interval, is what carries the
stated 6% floor.

## Reproducibility

Fitted by a versioned training protocol in
[stfc/goldilocks-ml](https://github.com/stfc/goldilocks-ml), from a sealed
dataset snapshot, with a fixed seed on CPU.

**The run reproduces bit for bit.** A second run over the same snapshot,
reusing the recorded split assignment, produced an identical estimator pickle,
model record, metrics and predictions — the same SHA-256 as the file in this
record. The decision rule reproduces with it: same quantile level, same band
offsets.
