...
| `k_index_dense` | Whether a structure needs rung 12 or above | [Dense-mesh screen](k_index_dense-screen.md) (draft) |
| `k_line_density` | k-points per unit reciprocal length | none |
| `k_pra` | k-points per reciprocal atom | none |

The dense-mesh screen advises no setting. It serves `goldilocks-data`: it
ranks structures nobody has calculated yet, so machine time spent extending
this dataset goes where its labels are thin. Core never reads it.
